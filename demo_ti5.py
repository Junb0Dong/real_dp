...
def main(output, robot_ip, vis_camera_idx, init_joints, frequency, command_latency):
    ...
    with SharedMemoryManager() as shm_manager:
        with KeystrokeCounter() as key_counter, \
            VisionPro(shm_manager=shm_manager) as vp, \
            RealEnv(
                output_dir=output, 
                robot_ip=robot_ip, 
            ...
            while not stop:
                ...
                vp_state = vp.get_left_wrist_state() #  获取左手腕状态
                # print(sm_state)
                dpos = vp_state[:3,3] * (env.max_pos_speed / frequency) # 从transformation matrix中提前取出平移部分并计算位置变化量
                drot = vp_state[:3,:3] # 从transformation matrix中提前取出旋转矩阵
                target_pose[:3] += dpos     #  将平移量加到目标姿态的前三个元素上
                target_pose[3:] = (drot * st.Rotation.from_rotvec(target_pose[3:])).as_rotvec()   #  将旋转矩阵与目标姿态的旋转部分相乘，得到新的旋转向量
