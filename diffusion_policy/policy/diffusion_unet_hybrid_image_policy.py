...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        for key, attr in obs_shape_meta.items(): #  遍历观察形状元数据
            shape = attr['shape'] #  获取形状
            obs_key_shapes[key] = list(shape) #  将形状存储到obs_key_shapes字典中
    ...
    def conditional_sample(self, 
        ...
        trajectory = torch.randn( #  生成与condition_data形状相同的随机噪声
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        B, To = value.shape[:2] #  获取观测值的形状
        T = self.horizon #  获取预测的时间步长
