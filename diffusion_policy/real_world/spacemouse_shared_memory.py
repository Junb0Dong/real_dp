...
class Spacemouse(mp.Process):
    def __init__(self, 
            shm_manager, 
            get_max_k=30,   # 从环形缓冲区中获取数据的最大尝试次数，默认为 30
            frequency=200,  
            max_value=500, 
            deadzone=(0,0,0,0,0,0),     # 死区值，用于过滤掉小于该值的输入，避免微小的抖动影响，默认为 (0,0,0,0,0,0)
            dtype=np.float32,
            n_buttons=2,
            ):
        """
        ...
        *----->x right
        ...
        """
        super().__init__()
        if np.issubdtype(type(deadzone), np.number): #  判断deadzone的类型是否为数字
            deadzone = np.full(6, fill_value=deadzone, dtype=dtype) #  如果是数字，则将deadzone赋值为一个长度为6的数组，数组中的每个元素都为deadzone
        else:
            deadzone = np.array(deadzone, dtype=dtype) #  如果不是数字，则将deadzone赋值为一个数组，数组中的元素类型为dtype, dtype默认为np.float32
        assert (deadzone >= 0).all() #  断言deadzone中的所有元素都大于等于0
        ...
        self.tx_zup_spnav = np.array([ #  定义一个3x3的数组，用于表示z轴向上的旋转矩阵
            [0,0,-1],
            [1,0,0],
            [0,1,0]
        ], dtype=dtype)
    ...
    def get_motion_state(self):
        state = self.ring_buffer.get()  # 从共享缓存区中获取数据
        state = np.array(state['motion_event'][:6], 
            dtype=self.dtype) / self.max_value  #  归一化处理
        is_dead = (-self.deadzone < state) & (state < self.deadzone)    #  死区检测
        state[is_dead] = 0
        return state
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if isinstance(event, SpnavMotionEvent):
                    ...
                    motion_event[6] = event.period  # 运动事件的周期
