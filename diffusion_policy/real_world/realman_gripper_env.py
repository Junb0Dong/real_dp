...
class RealmanGripperEnv:
    def __init__(self, 
            ...
            n_obs_steps=2,  # 观测步骤
            # obs
            obs_image_resolution=(424,240), # image分辨率
            max_obs_buffer_size=30, # 最大观测缓冲区大小
            camera_serial_numbers=None,
            obs_key_map=DEFAULT_OBS_KEY_MAP,    # obs观测字典
            obs_float32=False,
            # robot
            init_joints=None,  # joint 初始化
            target_pose_init=None,
            # video capture params
            video_capture_fps=15,   # video的fps
            video_capture_resolution=(640,480), # video_capture_resolution作为输入分辨率，obs_image_resolution作为输出分辨率
            # saving params
        ...
        robot = RTDERealmanGripperController(
            shm_manager=shm_manager,
            robot_ip=robot_ip,
            frequency=15, # UR5 CB3 RTDE
            joints_init=init_joints, # 关节初始位置
            init_target_pose = target_pose_init, # TODO：需要指定吗？
        )
    ...
    def get_obs(self) -> dict:
        ...
        for k, v in last_robot_data.items():
            if k in self.obs_key_map:   # 对robot的obs进行map，名字变换一下
    ...
    def exec_actions(self, 
            ...
            close_gripper_state: Optional[bool]=False):
        assert self.is_ready
        ...
        new_timestamps = timestamps[is_new] # TODO：源代码是给pose插值规划的时间，看一下还有没有其他的作用？
